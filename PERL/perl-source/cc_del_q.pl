...
#
`clearprompt yes_no -type warning -mask yes,no -default no -prompt "This will delete view-private files prompting for confirmation of each file. Do you wish to continue ?"`;
#
if ($? == 1)
{
    exit(0);
...
#
    if (length($') == 0)
    {
        `clearprompt yes_no -type ok -mask yes,no -default no -prompt "Delete    this file $File ?"`;
#
        if ($? == 0)
        {
# 
