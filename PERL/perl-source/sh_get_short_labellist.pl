...
foreach $Vob (@VobListArray)
{
    ...
    $VobText = $Vob;
    $VobText =~ s/\\//;
    $LabelListCommand = "cleartool lstype -kind lbtype -invob ".$Vob." -fmt \"%En\n\"";
...
#
    foreach $LabelLine (@LabelLineArray)
    {
        ...
        split(/ /, $LabelLineCopy);
        print @_[0]."\n";
        if ((($LabelLine cmp "LATEST") != 0) && (($LabelLine cmp "CHECKEDOUT") != 0))
        {
            print $LabelLine."\n";
