#------------------------------------------------------------------------------
#------ PERL SCRIPT  : cc_add_q.pl
#------ Authors Name : Mark Roberts
#------ Date         : 27 November, 1996.
#------ Description  : This script adds files, recursively,  that are in 
#                      view-private-storage to the VOB as VOB objects. This 
#                      version of the script propts the user to confirm each
#                      file.
# 
...
#
    if (length($') == 0)
    {
        `clearprompt yes_no -type ok -mask yes,no -default no -prompt "Add this file $File ?"`;
#
        if ($? == 0)
        {
# 
