...
@FlexLicenses = qw(RationalSuiteEnterprise ClearQuestMultiSite ClearQuest);
...
@LicenseAbreviations = qw(SE CQMS CQ);
...
# ClearCase block - remove if ClearCase is not used.
# print "=========================================================\n";
ClearDataStructures();
...
sub GetSuiteLicenseUse{
    ...
    foreach $_ (@LicenseArray)
    {
        ...
        #
        if (((m/\(Total of/) && (m/licenses available/)) || ((m/\(Total of/) && (m/licenses issued/))) 
        {
...
sub UpdateUseStats
{
    ...
    if (-e $StatsFileName)
    {
        ...
        while (!eof(STATS_FILE)) { read STATS_FILE, $Buffer, 25; $FileData .= $Buffer; }
        close STATS_FILE;
        ...
        {
            print $Date.".... ";
            for $Person (sort keys %{$Data{$Date}}) { print $Person." = ".$Data{$Date}{$Person}." "; }
            print "\n";
        }
    }
    ...
    foreach $Use (@UseArray)
    {
        ...
        {
            if (m/$CurrentName/) { $NameFound = 1; }
        }
        if ($NameFound == "0")
        {
            # Add the new name to the hash.
            for $Date (sort keys %Data) { $Data{$Date}{$_} = 0; }
        }
    }
    ...
    foreach $Use (@UseArray)
    {
        ...
        if ($Data{$DateSampled}{$_} == "") { $Data{$DateSampled}{$_} = "1"; }
        else { $Data{$DateSampled}{$_}++; }
    }
    ...
    {
        $Total = 0;
        for $Date (sort keys %Data) { $Total += $Data{$Date}{$Person}; }
        $Totals{$Person} = $Total;
    }
    ...
    for $Person (sort keys %{$Data{$LastDate}}) { print FILEHANDLE ",".$Person; }
    print FILEHANDLE "\n";
    for $Date (sort SortDates (keys %Data))
    {
        print FILEHANDLE $Date;
        for $Person (sort keys %{$Data{$Date}}) { print FILEHANDLE ",".$Data{$Date}{$Person}; }
        print FILEHANDLE "\n";
    }
    print FILEHANDLE "Totals";
    for $Person (sort keys %{Totals}) { print FILEHANDLE ",".$Totals{$Person}; }
    print FILEHANDLE "\n";
