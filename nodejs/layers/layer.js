...
var versionID = "v2"; // process.env.VERSION_ID;
var verbose = process.env.VERBOSE;
var ignoreDelaysFlag = false;
var skipCounter = 0;
var blockStartTime = new Date();
var timeBlockCounter = 0;
...
app.get('/get-results', (request, response) => {
    messageText = "";
    for (const key in resultCollection) {
      if (resultCollection.hasOwnProperty(key)) {
        var resultCollectionToProcess = resultCollection[key];

        let sortedResultCollection = {};

        Object.keys(resultCollectionToProcess).sort().forEach(key => {
          sortedResultCollection[key] = resultCollectionToProcess[key];
        });
        ...
        messageText = messageText + sprintfJS.sprintf("%02d : ", key) + resultCollectionTimeSlots[key] + " ---> "; 
        for (const secondKey in sortedResultCollection) {
          if (verbose == true) {
            console.log(`${secondKey} : ${sortedResultCollection[secondKey]}`);
          }
          messageText = messageText + `${secondKey} : ` + sprintfJS.sprintf("%-3d", sortedResultCollection[secondKey]) + " ";
        }
...
app.get('/reset-results', (request, response) => {
    messageText = "reset results to zero";
    resultCollection = {};
    resultHash = {};
    resultCollectionTimeSlots = {};
    timeBlockCounter = 0;
...
function recordResponses(name, version) {
    ...
    if (verbose == true) {
      console.log(blockStartTime.getMinutes() + ":" + blockStartTime.getSeconds() + "  " + dateNow.getMinutes() + ":" + dateNow.getSeconds());
    }

    var integerDateNow = dateNow.getTime() / 1000;
    var blockStartTimePlusInterval = (blockStartTime / 1000) + 10;

  if (blockStartTimePlusInterval > integerDateNow) {
    if (verbose == true) {
      console.log("block - " + timeBlockCounter + " ");
    }
    resultCollection[timeBlockCounter] = resultHash;
    resultCollectionTimeSlots[timeBlockCounter] = sprintfJS.sprintf("%02d:%02d:%02d - %02d:%02d:%02d", blockStartTime.getHours(), blockStartTime.getMinutes(), blockStartTime.getSeconds(), dateNow.getHours(), dateNow.getMinutes(), dateNow.getSeconds());
    if (verbose == true) {    
      console.log(resultCollectionTimeSlots[timeBlockCounter]);
    }
  } else {
    resultCollection[timeBlockCounter] = resultHash;
    resultCollectionTimeSlots[timeBlockCounter] = sprintfJS.sprintf("%02d:%02d:%02d - %02d:%02d:%02d", blockStartTime.getHours(), blockStartTime.getMinutes(), blockStartTime.getSeconds(), dateNow.getHours(), dateNow.getMinutes(), dateNow.getSeconds());
